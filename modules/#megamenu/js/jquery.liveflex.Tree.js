	...
	$.liveflex.treeview = function(el, options) {
		...
		base.init = function() {
			...
			if (base.options.drag) {
				base.$el.find('li ' + base.options.handle).livequery(function(){
				 $(this).droppable({
					tolerance: "pointer",
					...
					drop: function(event, ui) {
						...
						switch (base.pos.target()) {
							...
							case 'above':
							  if(me.attr('id') == "home") {
							    alert('You can have only one root element');
							  } else {
  								//add before
				...
				base.$el.find('li.custom').livequery(function(){
				  $(this).draggable({
					opacity: 0.3,
					revert: true,
					delay: 300,
		...
		base.getMoved = function() {
			...
			base.$el.find('.moved').each(function(i) {
				...
				var pos = $(li).prev('li').attr('pos');
				if( pos == undefined ) {
					if( $(li).parent().closest('li').hasClass('custom') == true) {
						pos = 1;
					} else {
						pos = $(li).parent().closest('li').attr('pos');
					}		    	
				}
				var position = parseFloat(pos) + 0.01;	
				$(li).attr('pos', position);
				
        var custom = li.parent().closest('li').hasClass('custom');
        result += li.attr('data-id') + '/' + parentli.attr('data-id') + '/' + position + '/' + custom + ',';
			});
			return result;
		};
