...
{/if}
{assign var=product_smartprice value=0}
...
<tr class="product-line-row">
	...
	<td class="productQuantity text-center">
		<span class="product_quantity_show{if (int)$product['product_quantity'] > 1} badge{/if}">{convertQty quantity={$product['product_quantity']}}</span>
		{if $can_edit}
		<span class="product_quantity_edit" style="display:none;">
			<input type="text" name="product_quantity" class="edit_product_quantity" value="{convertQty quantity=$product['product_quantity']}"/>
			{if $product.pp_qty_step > 0}<span class="pp_qty_step">{l s='quantity step %1s' sprintf=$product.pp_qty_step|formatQty}</span>{/if}
		</span>
		{/if}
		{displayQty quantity=$product['product_quantity'] m='unit'}
	</td>
	...
	{/if}
	{if $stock_management}<td class="productQuantity product_stock text-center">{displayQty quantity=$product['current_stock'] m='inline'}</td>{/if}
	<td class="total_product">
		{displayPrice price=(PP::calcPrice($product_price, ($product['product_quantity'] - $product['customizationQuantityTotal']), $product['product_quantity_fractional'], $product)+$product_smartprice) currency=$currency->id m='total'}
	</td>
