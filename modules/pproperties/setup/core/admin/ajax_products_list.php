...
$sql = 'SELECT p.`id_product`, pl.`link_rewrite`, p.`reference`, pl.`name`, MAX(image_shop.`id_image`) id_image, il.`legend`, p.`cache_default_attribute`
		...
		' AND p.id_product NOT IN (SELECT pp.id_product FROM `'._DB_PREFIX_.'product` pp WHERE (pp.id_product = p.id_product AND p.id_pp_template > 0 AND (2 IN (SELECT pt.qty_policy FROM `'._DB_PREFIX_.'pp_template` pt WHERE pt.id_pp_template = p.id_pp_template))))';
