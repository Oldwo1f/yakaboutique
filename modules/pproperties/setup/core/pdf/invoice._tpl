	...
	<tr>
		...
		<td style="width: 83%; text-align: right">
			<table style="width: 100%; font-size: 8pt;">
				...
				{ppAssign order=$order_detail currency=$order->id_currency pdf=true}
				<tr style="line-height:6px;background-color:{$bgcolor};" {if Configuration::get('PS_PDF_IMG_INVOICE') && isset($order_detail.image) && $order_detail.image->id && isset($order_detail.image_size)}height="{$order_detail['image_size'][1]}"{/if}>
					...
					{/if}
					<td style="text-align: left; width: {$product_reference_width}%">{$order_detail.product_name}{if isset($order_detail.product_reference) && !empty($order_detail.product_reference)} ({l s='Reference:' pdf='true'} {$order_detail.product_reference}){/if}{displayProductName name=''}{*[HOOK ppropertiessmartprice]*}</td>
					<!-- unit price tax excluded is mandatory -->
					{if !$tax_excluded_display}
					...
					<td style="text-align: center; width: 15%;">{displayQty quantity=$order_detail.product_quantity}</td>
					<td style="text-align: right; width: {if !$tax_excluded_display}10%{else}20%{/if}; white-space: nowrap;">
					{if $tax_excluded_display}
						{displayPrice currency=$order->id_currency price=$order_detail.total_price_tax_excl m='total'}
					{else}
						{displayPrice currency=$order->id_currency price=$order_detail.total_price_tax_incl m='total'}
					{/if}
					</td>
				</tr>
					{foreach $order_detail.customizedDatas as $customizationPerAddress}
						{foreach $customizationPerAddress as $customizationId => $customization}
							{if $order_detail.id_cart_product == $customization.id_cart_product}
