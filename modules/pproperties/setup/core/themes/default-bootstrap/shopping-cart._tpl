		...
		<table id="cart_summary" class="table table-bordered {if $PS_STOCK_MANAGEMENT}stock-management-on{else}stock-management-off{/if}">
			...
			<tbody>
				...
				{foreach $products as $product}
					...
					{assign var='ignoreProductLast' value=($productHasCustomizedDatas && isset($customizedDatas.$productId.$productAttributeId)) || count($gift_products)}
					{* Display the product line *}
					{include file="$tpl_dir./shopping-cart-product-line.tpl" productLast=$product@last productFirst=$product@first}
					{* Then the customized datas ones*}
					{if $productHasCustomizedDatas && isset($customizedDatas.$productId.$productAttributeId)}
						...
						{if $product.id_cart_product == $customization.id_cart_product}
							<tr
								...
								class="{$icpClass} product_customization_for_{$product.id_product}_{$product.id_product_attribute}_{$product.id_address_delivery|intval} {if $odd}odd{else}even{/if} customization alternate_item {if $product@last && $customization@last && !count($gift_products)}last_item{/if}">
								<td></td>
								<td colspan="6">
				...
				{foreach $gift_products as $product}
					...
					{assign var='ignoreProductLast' value=($productHasCustomizedDatas && isset($customizedDatas.$productId.$productAttributeId))}
