...
*
* --- DO NOT REMOVE OR MODIFY THIS LINE PP_VERSION[1.6.0.14] PP_VERSION_REQUIRED[1.6.0.9#6] ---
*
...
{assign var='productProperties' value=PP::getProductPropertiesByTemplateId($product.id_pp_template)}
{assign var='productHasCustomizedDatas' value=Product::hasCustomizedDatas($product, $customizedDatas)}
	{ldelim}
		...
		"icp": {$product.id_cart_product},
		"css": "{$product|pp:css:false}",
		"cart_quantity_fractional": {$product.cart_quantity_fractional|floatval},
		"pp_qty_text": "{$productProperties.pp_product_qty_text|pp_safeoutput:js}",
		"pp_ext": {$productProperties.pp_ext},
{if $productProperties.pp_ext}
		"pp_ext_prop_data": {PP::productExtProperties($product)|json_encode},
{/if}
...
{else}
		"hasAttributes": false,
{/if}
		"hasCustomizedDatas": {if $productHasCustomizedDatas}true{else}false{/if},

		"customizedDatas":[
		{if $productHasCustomizedDatas}
		{assign var='productWithCustomizedDatasAdded' value=false}
		...
		{if $product.id_cart_product == $customization.id_cart_product}
			{if $productWithCustomizedDatasAdded},{/if}{assign var='productWithCustomizedDatasAdded' value=true}
			{ldelim}
