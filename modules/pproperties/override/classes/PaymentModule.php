				...
				if (!$order_creation_failed && isset($order->id))
				{
					...
					foreach ($order->product_list as $product)
					{
						...
						$product_var_tpl = array(
							...
							'name' => $product['name'].(isset($product['attributes']) ? ' - '.$product['attributes'] : '').PP::smartyDisplayProductName(array('name' => '')).$ppropertiessmartprice_hook2,
							'unit_price' => PP::smartyDisplayPrice(array('price' => (Product::getTaxCalculationMethod() == PS_TAX_EXC ? Tools::ps_round($price, 2) : $price_wt))),
							'price' => PP::smartyDisplayPrice(array('price' => (Product::getTaxCalculationMethod() == PS_TAX_EXC ? $product['total'] : $product['total_wt']), 'quantity' => (int)$product['cart_quantity'], 'm' => 'total')),
							'quantity' => PP::smartyDisplayQty(array('quantity' => (int)$product['cart_quantity'])),
							'customization' => array()
						);
							...
							foreach ($customized_datas[$product['id_product']][$product['id_product_attribute']][$order->id_address_delivery] as $customization)
							{
								if ($product['id_cart_product'] == $customization['id_cart_product'])
								{
									...
									$product_var_tpl['customization'][] = array(
										...
										'customization_quantity' => PP::smartyDisplayQty(array('quantity' => $customization_quantity)),
										'quantity' => PP::smartyDisplayPrice(array('price' => (Product::getTaxCalculationMethod() == PS_TAX_EXC ? $product['total_customization'] : $product['total_customization_wt']), 'm' => 'total'))
									);
								}
							}
						}
