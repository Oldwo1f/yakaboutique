...
class MegaMenu extends Module
{
  ...
  public function install()
  {
    ...
    if (!parent::install() OR
      ...
      !Configuration::updateValue('MEGAMENU_IMAGETYPE', 'custom') OR
	  //global
  ...
  public function uninstall()
  {
    ...
    if (!parent::uninstall() OR
      ...
      !Configuration::deleteByName('MEGAMENU_IMAGETYPE') OR
  ...
  public function getContent()
  {
    ...
    if (Tools::isSubmit('submitMegaMenu'))
    {
		...
		Configuration::updateValue('MEGAMENU_IMAGETYPE', Tools::getValue('MEGAMENU_IMAGETYPE'));
         ...
         jq162(document).ready(function() {
            jq162("#file_upload").uploadify({
              ...
              onComplete: function(event, queueID, fileObj, response, data){
                    alert("You have uploaded " + fileObj.name );
                }
            });
  ...
  public function displaySettingsForm()
  {
    ...
    <form action="'.$_SERVER['REQUEST_URI'].'" method="post" style="">
        ...
        <div class="form-group">
            ...
            <div class="margin-form col-lg-9">
                <select name="MEGAMENU_ROWITEMS" style="width:170px;">
                    ...
                    <option value="6" '.(Configuration::get('MEGAMENU_ROWITEMS') == 6 ? 'selected="selected"' : '').'>'.$this->l('6').'</option>
                    </select>
            </div>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            <label class="control-label col-lg-3">'.$this->l('Image type :').'</label>
            <div class="margin-form col-lg-9">
                <select name="MEGAMENU_IMAGETYPE" style="width:170px;">
                    <option value="custom" '.((int)(Configuration::get('MEGAMENU_IMAGETYPE')) == 'custom' ? 'selected="selected"' : '').'>'.$this->l('use uploaded images').'</option>';

                    $imagesTypes = ImageType::getImagesTypes('categories');
                foreach($imagesTypes as $imageType){
                     $output .= '<option value="'.$imageType['name'].'" '.(Configuration::get('MEGAMENU_IMAGETYPE') == $imageType['name'] ? 'selected="selected"' : '').'>'.$imageType['name'].'</option>';
                    }
  ...
  public function getTree($resultParents, $resultIds, $maxDepth,  $id_category = 1, $image_type, $currentDepth = 0)
  {
    $children = array();
    if (isset($resultParents[$id_category]) AND sizeof($resultParents[$id_category]) AND ($maxDepth == 0 OR $currentDepth < $maxDepth)){
        ...
        {
            $children[] = $this->getTree($resultParents, $resultIds, $maxDepth, $subcat['id_category'], $image_type, $currentDepth + 1);
        }
        if( $currentDepth == 2 ){
    ...
    if (!isset($resultIds[$id_category])){
        ...
        return array('id' => $id_category, 
            'link'=> $resultIds[$id_category]['url'],
           'name' => $resultIds[$id_category]['name'], 
           'desc'=> $resultIds[$id_category]['title'],
           'children' => $children, 
           'currentDepth' => $currentDepth, 
           'image' => _MODULE_DIR_.'megamenu/images/'.$id_category.'.png',
        ...
        {
            if(file_exists(dirname(__FILE__).'/../../img/c/'.$id_category.'-'.$image_type.'.jpg'))
                $imagesize = getimagesize(dirname(__FILE__).'/../../img/c/'.$id_category.'-'.$image_type.'.jpg' );
            
            $image = Tools::getHttpHost(true).__PS_BASE_URI__.'/img/c/'.$id_category.'-'.$image_type.'.jpg';
        }
  ...
  */
  ...
  $blockCategTree = null;
	if (version_compare(_PS_VERSION_, '1.5.0.0')  == +1) {
        ...
        $blockCategTree = $this->getTree($resultParents, $resultIds, 3, (int)Configuration::get('PS_HOME_CATEGORY'), Configuration::get('MEGAMENU_IMAGETYPE'));
        unset($resultParents);
        unset($resultIds);
