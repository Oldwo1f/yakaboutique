...
class WishList extends ObjectModel
{
	...
	public static function getProductByIdCustomer($id_wishlist, $id_customer, $id_lang, $id_product = null, $quantity = false)
	{
		...
		SELECT wp.`id_product`, wp.`quantity`, wp.`quantity_fractional`, (p.`quantity` + p.`quantity_remainder`) as product_quantity, pl.`name`, wp.`id_product_attribute`, wp.`priority`, pl.link_rewrite, cl.link_rewrite as category_rewrite
		...
		for ($i = 0; $i < $count; ++$i)
		{
			...
			{
				$result = Db::getInstance()->executeS('
				SELECT al.`name` as attribute_name, (pa.`quantity` + pa.`quantity_remainder`) as "attribute_quantity"
			...
			Product::amendProduct($products[$i]);
		}
		return ($products);
	}
	...
	public static function addProduct($id_wishlist, $id_customer, $id_product, $id_product_attribute, $quantity)
	{
		if (!Validate::isUnsignedId($id_wishlist) ||
			...
			!Validate::isUnsignedFloat($quantity))
			die (Tools::displayError());
		...
		if (empty($result) === false && count($result))
		{
			...
			if (($qty + $quantity) <= 0)
				...
				`quantity_fractional` = '.($qty_policy_legacy ? 0 : (float)($quantity + $qty)).'
				WHERE `id_wishlist` = '.(int)($id_wishlist).'
			...
			'.($qty_policy_legacy ? 0: (float)($quantity)).',
			 1)'));
	...
	public static function updateProduct($id_wishlist, $id_product, $id_product_attribute, $priority, $quantity)
	{
		...
		`quantity` = '.($qty_policy_legacy ? (int)$quantity : 1).',
		`quantity_fractional` = '.($qty_policy_legacy ? 0 : (float)$quantity).'
		WHERE `id_wishlist` = '.(int)($id_wishlist).'
		AND `id_product` = '.(int)($id_product).'
		AND `id_product_attribute` = '.(int)($id_product_attribute)));
	}
	...
	public static function addBoughtProduct($id_wishlist, $id_product, $id_product_attribute, $id_cart, $quantity)
	{
		...
		$qty = ($qty_policy_legacy ? (int)$result['quantity'] : (float)$result['quantity_fractional']);
		if (!count($result) ||
			($qty - $quantity) < 0 ||
			$quantity > $qty)
			return (false);
