...
*
* --- DO NOT REMOVE OR MODIFY THIS LINE PP_VERSION[1.6.0.14] PP_VERSION_REQUIRED[1.6.0.11] ---
*
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		$('.delete_product').live('click', function(e) {
			...
			deleteProduct(to_delete[1], to_delete[2], to_delete[3], to_delete[4]);
		});
		...
		$('.cart_quantity').live('change', function(e) {
			...
			if ($(this).val() != cart_quantity[$(this).attr('rel')])
			{
				...
				updateQty(product[0], product[1], product[2], $(this).val() - cart_quantity[$(this).attr('rel')], product[3]);
			}
		});
		$('.increaseqty_product, .decreaseqty_product').live('click', function(e) {
			...
			if ($(this).hasClass('decreaseqty_product'))
				sign = '-';
			updateQty(product[0], product[1], product[2], sign+1, product[3]);
		});
	...
	function displayQtyInStock(id)
	{
		...
		$('.pp_bo_qty_text').html(pproperties[id_product]['pp_bo_qty_text']);	
	}
	...
	function deleteProduct(id_product, id_product_attribute, id_customization, id_cart_product)
	{
		$.ajax({
			type:"POST",
			url: "{$link->getAdminLink('AdminCarts')|addslashes}",
			...
			success : function(res)
			{
				...
				if(res.found)
				{
					...
					$.each(res.products, function() {
						...
						pproperties[this.id_product]  = this.pproperties;
	...
	function updateCartProducts(products, gifts, id_address_delivery)
	{
		...
		$.each(products, function() {
			...
			cart_quantity[Number(this.id_product)+'_'+Number(this.id_product_attribute)+'_'+Number(this.id_customization)+'_'+Number(this.id_cart_product)] = this.cart_quantity;
			cart_content += '<tr><td><img src="'+this.image_link+'" title="'+this.name+'" /></td><td>'+this.name+'<br />'+this.attributes_small+'</td><td>'+this.reference+'</td><td><input type="text" rel="'+this.id_product+'_'+this.id_product_attribute+'" class="product_unit_price" value="'+this.numeric_price+ '" />&nbsp;'+pp.priceText(this)+'</td><td>';
			cart_content += (!this.id_customization ? '<div class="input-group fixed-width-md"><div class="input-group-btn"><a href="#" class="btn btn-default increaseqty_product" rel="'+this.id_product+'_'+this.id_product_attribute+'_'+(this.id_customization ? this.id_customization : 0)+'_'+this.id_cart_product+'" ><i class="icon-caret-up"></i></a><a href="#" class="btn btn-default decreaseqty_product" rel="'+this.id_product+'_'+this.id_product_attribute+'_'+(this.id_customization ? this.id_customization : 0)+'_'+this.id_cart_product+'"><i class="icon-caret-down"></i></a></div>' : '');
			cart_content += (!this.id_customization ? '<input type="text" rel="'+this.id_product+'_'+this.id_product_attribute+'_'+(this.id_customization ? this.id_customization : 0)+'_'+this.id_cart_product+'" class="cart_quantity" value="'+this.cart_quantity+'" />' : '');
			cart_content += (!this.id_customization ? '<div class="input-group-btn"><a href="#" class="delete_product btn btn-default" rel="delete_'+this.id_product+'_'+this.id_product_attribute+'_'+(this.id_customization ? this.id_customization : 0)+'_'+this.id_cart_product+'" ><i class="icon-remove text-danger"></i></a></div></div><span>'+pp.qtyText(this)+'</span>' : '');
	...
	function updateQty(id_product, id_product_attribute, id_customization, qty, id_cart_product)
	{
		$.ajax({
			type:"POST",
			url: "{$link->getAdminLink('AdminCarts')|addslashes}",
	...
	function addProduct()
	{
		...
		{
			$('#products_err').addClass('hide');
			updateQty(id_product, $('#ipa_'+id_product+' option:selected').val(), 0, $('#qty').val(), 'add');
		}
	}
			...
			<div class="form-group">
				...
				<div class="col-lg-9">
					<div class="input-group fixed-width-md">
						...
						<span class="input-group-addon pp_bo_qty_text"></span>
					</div>
					<p class="help-block">{l s='In stock'} <span id="qty_in_stock"></span> <span class="pp_bo_qty_text"></span></p>
				</div>
			</div>
