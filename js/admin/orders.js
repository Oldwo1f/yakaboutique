...
function addProductRefreshTotal()
{
	...
	if (price < 0 || isNaN(price))
		price = 0;
	var total = ppMakeTotalProductCaculation(typeof element == "undefined" ? null : element, quantity, price);
	...
	if (element_list.length)
	{
		...
		$.each(element_list, function(i, elm) {
			if($(elm).find('.edit_product_quantity').length)
			{
				...
				subtotal = ppMakeTotalProductCaculation(typeof element == "undefined" ? null : element, $(elm).find('.edit_product_quantity').val(), price);
				$(elm).find('.total_product').html(formatCurrency(subtotal, currency_format, currency_sign, currency_blank));
			}
		});

		var total = ppMakeTotalProductCaculation(typeof element == "undefined" ? null : element, qty, price);
...
function init()
{
	...
	$('input#add_product_product_quantity').unbind('keyup').keyup(function() {
		if (stock_management)
		{
			...
			if (quantity < 0 || isNaN(quantity))
				quantity = 1;
			var stock_available = pp.parseFloat($('#add_product_product_stock').html());
			// stock status update
	...
	$('.edit_product_quantity').keyup(function() {
		...
		if (quantity < 0 || isNaN(quantity))
			quantity = 1;
		var stock_available = pp.parseFloat($(this).parent().parent().parent().find('td.product_stock').html());
		// total update
		editProductRefreshTotal($(this));
	});
