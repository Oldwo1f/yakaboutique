		...
		for ($current_line = 0; $line = fgetcsv($handle, MAX_LINE_SIZE, $this->separator); $current_line++)
		{
			...
			else if (isset($category->parent) && is_string($category->parent))
			{
				...
				else
				{
					...
					$category_to_create->id_parent = Configuration::get('PS_HOME_CATEGORY'); 					if (($field_error = $category_to_create->validateFields(UNFRIENDLY_ERROR, true)) === true &&
	...
	*/
	public function productImport()
	{
		...
		for ($current_line = 0; $line = fgetcsv($handle, MAX_LINE_SIZE, $this->separator); $current_line++)
		{
			...
			{
				$product->id_category = array(); 				foreach ($product->category as $value)
				{
					if (is_numeric($value))
					{
						...
						else
						{
							...
							$category_to_create->active = 1;
							$category_to_create->id_parent = Configuration::get('PS_HOME_CATEGORY'); 							$category_link_rewrite = Tools::link_rewrite($category_to_create->name[$default_language_id]);
							$category_to_create->link_rewrite = AdminImportController::createMultiLangField($category_link_rewrite);
