...
var ajaxCart = {
	...
	overrideButtonsInThePage : function(){
		...
		$(document).off('click', '.ajax_add_to_cart_button').on('click', '.ajax_add_to_cart_button', function(e){
			...
			if ($(this).prop('disabled') != 'disabled')
				ajaxCart.add(idProduct, null, false, this, 'default');
		});
		...
		$(document).off('click', '.cart_block_list .ajax_cart_block_remove_link').on('click', '.cart_block_list .ajax_cart_block_remove_link', function(e){
			...
			if (!customizationId)
			{
				...
				icp = ppCart.getIcp($(this).parent().parent());
			}
	...
	add : function(idProduct, idCombination, addedFromProductPage, callerElement, quantity, whishlist){
		...
		$.ajax({
			...
			cache: false,
			dataType : "json",
			data: 'controller=cart&add=1&ajax=true&qty=' + quantity + '&id_product=' + idProduct + '&token=' + static_token + ( (parseInt(idCombination) && idCombination != null) ? '&ipa=' + parseInt(idCombination) : '') + pp_ext,
	...
	updateProductQuantity : function (product, quantity){
		$('dt[data-id=cart_block_product_' + product.id + '_' + (product.idCombination ? product.idCombination : '0')+ '_' + (product.idAddressDelivery ? product.idAddressDelivery : '0') + ']' + ppCart.getIcpSelector(product.icp) + ' .quantity').fadeTo('fast', 0, function() {
			...
			if (product.cart_quantity_fractional > 0)
				$(this).parents('.product-name').first().find('.pp_qty_wrapper .pp_qty').text(pp.formatQty(product.cart_quantity_fractional));
		...
		$(jsonData.products).each(function(){
			...
			if (this.id != undefined)
			{
				...
				if ($('dt[data-id="cart_block_product_' + domIdProduct + '"]' + icp).length == 0)
				{
					...
					ppCart.amendProduct(this, $('.cart_block'));
				}
	...
	updateCartEverywhere : function(jsonData){
		...
		$('#cart_blockuserinfo').text($.trim(jsonData.productTotal));
