...
*
* --- DO NOT REMOVE OR MODIFY THIS LINE PP_VERSION[1.6.0.14] PP_VERSION_REQUIRED[1.6.0.11#3] ---
*
...
$(document).ready(function(){
	$('.cart_quantity_up').off('click').on('click', function(e){
		e.preventDefault();
		upQuantity($(this), $(this).attr('id').replace('cart_quantity_up_', ''));
	});
	$('.cart_quantity_down').off('click').on('click', function(e){
		e.preventDefault();
		downQuantity($(this), $(this).attr('id').replace('cart_quantity_down_', ''));
	});
	$('.cart_quantity_delete' ).off('click').on('click', function(e){
		e.preventDefault();	
		deleteProductFromSummary($(this), $(this).attr('id'));
	});
	...
	$('.cart_quantity_input').off('keyup').on('keyup', function(e){
		e.preventDefault();
		updateQty($(this), this.value, true, this);
	});
...
function updateQty(o, val, cart, el)
{
	var id = $(el).attr('name');
	var icp = ppCart.getIcp(o);
	var qty_behavior = ppCart.qtyBehavior(ppCart.products[icp]);

	var observer = o.data('observer');
	if (observer == undefined) {
		observer = pp.observer(
			function (observer) {
				updateQty(o, el.value, cart, el);
			},
			function (observer) {
		...
		o.data('observer', observer);
	}

	var qty = (qty_behavior ? pp.parseFloat(val) : parseInt(val));
	if (qty > 0) {
		if (!observer.timeout && observer.observe(val, qty))
			return;
		var prefix = "";
		...
		if (qty_behavior) {
			if (pp.normalizeFloatAsString(val) == pp.normalizeFloatAsString(qty)) {
				if (ppCart.products[icp]['pp_qty_policy'] != 2) {
					qty = Math.floor(qty);
				}
				if (ppCart.products[icp]['pp_qty_step'] > 0) {
					var q = pp.processQtyStep(qty, ppCart.products[icp]['pp_qty_step']);
					if (!observer.timeout && observer.observe(q, qty))
						return;
					qty = q;
				}
				if (qty.toFixed(8) != ppCart.products[icp].cart_quantity_fractional.toFixed(8))
					upQuantity(o, id.replace('quantity_', ''), qty, true);
				else
					$(prefix+'input[name='+ id +']'+icpSelector).val($(prefix+'input[name='+ id +'_hidden]'+icpSelector).val());
			}
			else
				$(prefix+'input[name='+ id +']'+icpSelector).val($(prefix+'input[name='+ id +'_hidden]'+icpSelector).val());
		}
			...
			{
				var hidden = $(prefix+'input[name='+ id +'_hidden]'+icpSelector).val();
				var input = $(prefix+'input[name='+ id +']'+icpSelector).val();
				var QtyToUp = parseInt(input) - parseInt(hidden);

				if (parseInt(QtyToUp) > 0)
					upQuantity(o, id.replace('quantity_', ''), QtyToUp);
				else if(parseInt(QtyToUp) < 0)
					downQuantity(o, id.replace('quantity_', ''), QtyToUp);
			}
			else
				$(prefix+'input[name='+ id +']'+icpSelector).val($(prefix+'input[name='+ id +'_hidden]'+icpSelector).val());
		}
...
function deleteProductFromSummary(o, id)
{
	var icp = (isNaN(o) ? ppCart.getIcp(o) : o);
	var icpSelector = ppCart.getIcpSelector(icp);
	...
	$.ajax({
		...
		success: function(jsonData)
		{
			...
			else
			{
				...
				{
					$('#product_'+ id + icpSelector).fadeOut('slow', function() {
						$(this).remove();
						cleanSelectAddressDelivery();
						if (!customizationId)
							refreshOddRow();
					...
					for (i=0;i<jsonData.summary.products.length;i++)
					{
						if (jsonData.summary.products[i].id_product == productId
							...
							&& jsonData.summary.products[i].id_cart_product === icp)
								exist = true;
					}
					// if all customization removed => delete product line
					if (!exist && customizationId)
						$('#product_' + productId + '_' + productAttributeId + '_0_' + id_address_delivery + icpSelector).fadeOut('slow', function() {
							$(this).remove();
...
function upQuantity(o, id, qty, update)
{
	...
	if (typeof(ids[3]) !== 'undefined')
		id_address_delivery = parseInt(ids[3]);
	if (update === true)
		o.blur().closest('.cart_quantity').find('.ajax-processing').show();
	$.ajax({
		type: 'POST',
		headers: { "cache-control": "no-cache" },
		url: baseUri + '?rand=' + new Date().getTime(),
		...
		data: 'controller=cart'
			+ '&ajax=true'
			+ (update === true ? '&update=true' : '&add=true')
		...
		success: function(jsonData)
		{
			o.closest('.cart_quantity').find('.ajax-processing').hide();
			if (jsonData.hasError)
			{
				var errors = '';
				for(var error in jsonData.errors)
				...
				if (!!$.prototype.fancybox)
					...
					alert(errors);
				$('input'+ icpSelector +'[name=quantity_'+ id +']').val($('input'+ icpSelector +'[name=quantity_'+ id +'_hidden]').val());
			}
			else
			{
				if (jsonData.refresh)
		...
		error: function(XMLHttpRequest, textStatus, errorThrown) {
			if (update === true)
				o.parent('.cart_quantity').find('.ajax-processing').hide();
			if (textStatus !== 'abort')
			{
...
function updateCartSummary(json)
{
	...
	for (i=0;i<json.products.length;i++)
		product_list[json.products[i].id_product+'_'+json.products[i].id_product_attribute+'_'+json.products[i].id_address_delivery+ppCart.getIcpSuffix(json.products[i].id_cart_product)] = json.products[i];

	if (!$('.multishipping-cart:visible').length)
	{
		for (i=0;i<json.gift_products.length;i++)
			if (typeof(product_list[json.gift_products[i].id_product+'_'+json.gift_products[i].id_product_attribute+'_'+json.gift_products[i].id_address_delivery+ppCart.getIcpSuffix(json.gift_products[i].id_cart_product)]) !== 'undefined')
				product_list[json.gift_products[i].id_product+'_'+json.gift_products[i].id_product_attribute+'_'+json.gift_products[i].id_address_delivery+ppCart.getIcpSuffix(json.gift_products[i].id_cart_product)].quantity -= json.gift_products[i].cart_quantity;
	}
	else
		for (i=0;i<json.gift_products.length;i++)
			if (typeof(product_list[json.gift_products[i].id_product+'_'+json.gift_products[i].id_product_attribute+'_'+json.gift_products[i].id_address_delivery+ppCart.getIcpSuffix(json.gift_products[i].id_cart_product)]) == 'undefined')
				product_list[json.gift_products[i].id_product+'_'+json.gift_products[i].id_product_attribute+'_'+json.gift_products[i].id_address_delivery+ppCart.getIcpSuffix(json.gift_products[i].id_cart_product)] = json.gift_products[i];

	for (i in product_list)
	{
		var icp = product_list[i].id_cart_product;
		var rowSelector = 'tr' + ppCart.getIcpSelector(icp) + ' ';
		...
		$(rowSelector + '#product_price_' + key_for_blockcart).html('<li class="' + current_price_class + '">' + current_price + '</li>' + initial_price_text);
		if (typeof(product_list[i].customizationQuantityTotal) !== 'undefined' && product_list[i].customizationQuantityTotal > 0)
			$(rowSelector + '#total_product_price_' + key_for_blockcart).html(formatCurrency(product_customization_total, currencyFormat, currencySign, currencyBlank));
		else
			$(rowSelector + '#total_product_price_' + key_for_blockcart).html(formatCurrency(product_total, currencyFormat, currencySign, currencyBlank));
		if (product_list[i].quantity_without_customization != product_list[i].quantity)
			$(rowSelector + '#total_product_price_' + key_for_blockcart_nocustom).html(formatCurrency(product_total, currencyFormat, currencySign, currencyBlank));
		$(rowSelector+'.cart_total .pp_qty').html(pp.formatQty(pp.getCartTotalQty(product_list[i]), ppCart.decimalSign));
		if (!pp.isPackCalculator(product_list[i])) {
			$(rowSelector+'.cart_total .pp_qty_wrapper')[product_list[i].cart_quantity > 1 ? 'show' : 'hide']();
		}
		$(rowSelector+'td.cart_quantity .pp_qty_wrapper')[product_list[i].cart_quantity_fractional > 0 ? 'show' : 'hide']();

		var qty_behavior = ppCart.qtyBehavior(ppCart.products[icp]);
		if (qty_behavior)
			ppCart.products[icp].cart_quantity_fractional = pp.parseFloat(product_list[i].cart_quantity_fractional);
		$(rowSelector+'.cart_quantity input').val(qty_behavior ? pp.formatQty(product_list[i].cart_quantity_fractional) : product_list[i].cart_quantity);
		...
		if (typeof(product_list[i].customizationQuantityTotal) !== 'undefined' && product_list[i].customizationQuantityTotal > 0)
			$(rowSelector+'#cart_quantity_custom_' + key_for_blockcart).html(product_list[i].customizationQuantityTotal);
		nbrProducts += parseInt(product_list[i].quantity);
		pp.hooks.call('smartprice', 'ppCart.amendProduct', {'product':product_list[i], 'self':$(rowSelector)});
	}
	...
	for(var i in json)
		for(var j in json[i])
			for(var k in json[i][j])
				...
				{
					var rowSelector = 'tr' + ppCart.getIcpSelector(json[i][j][k][l]['id_cart_product']) + '.customization ';
					var quantity = json[i][j][k][l]['quantity'];
					$(rowSelector + 'input[name=quantity_' + i + '_' + j + '_' + l + '_' + k + '_hidden]').val(quantity);
					$(rowSelector + 'input[name=quantity_' + i + '_' + j + '_' + l + '_' + k + ']').val(quantity);
				}
}
